...
internal class RgbFloat323232TiffColor<TPixel> : TiffBaseColorDecoder<TPixel> where TPixel : unmanaged, IPixel<TPixel>
{
	private readonly bool isBigEndian;

	public RgbFloat323232TiffColor(bool isBigEndian)
	{
		...
		for (int i = top; i < top + height; i++)
		{
			...
			if (isBigEndian)
			{
				for (int j = 0; j < span2.Length; j++)
				{
					...
					num += 4;
					Vector4 vector = new Vector4(x, y, z, 1f);
			...
			else
			{
				for (int k = 0; k < span2.Length; k++)
				{
					...
					num += 4;
					Vector4 vector2 = new Vector4(x2, y2, z2, 1f);
