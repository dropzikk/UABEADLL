...
internal class WhiteIsZero32FloatTiffColor<TPixel> : TiffBaseColorDecoder<TPixel> where TPixel : unmanaged, IPixel<TPixel>
{
	private readonly bool isBigEndian;

	public WhiteIsZero32FloatTiffColor(bool isBigEndian)
	{
		...
		for (int i = top; i < top + height; i++)
		{
			...
			if (isBigEndian)
			{
				for (int j = 0; j < span2.Length; j++)
				{
					...
					span.Reverse();
					float num2 = 1f - BitConverter.ToSingle(span);
					num += 4;
			...
			else
			{
				...
				{
					float num3 = 1f - BitConverter.ToSingle(data.Slice(num, 4));
					num += 4;
