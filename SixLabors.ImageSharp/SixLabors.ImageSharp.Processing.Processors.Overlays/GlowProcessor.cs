...
internal class GlowProcessor<TPixel> : ImageProcessor<TPixel> where TPixel : unmanaged, IPixel<TPixel>
{
	private readonly struct RowOperation : IRowOperation<float>
	{
		...
		public void Invoke(int y, Span<float> span)
		{
			...
			for (int i = 0; i < bounds.Width; i++)
			{
				...
				span[i] = Numerics.Clamp(blendPercent * (1f - 0.95f * (num / maxDistance)), 0f, 1f);
			}
	...
	{
		TPixel value = definition.GlowColor.ToPixel<TPixel>();
		...
		float num = definition.Radius.Calculate(rectangle.Size);
		float maxDistance = ((num > 0f) ? MathF.Min(num, (float)rectangle.Width * 0.5f) : ((float)rectangle.Width * 0.5f));
