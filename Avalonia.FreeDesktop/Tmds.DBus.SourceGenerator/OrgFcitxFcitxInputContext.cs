...
internal class OrgFcitxFcitxInputContext
{
	...
	public OrgFcitxFcitxInputContext(Connection connection, string destination, string path)
	{
	...
	public Task FocusInAsync()
	{
		...
		MessageBuffer CreateMessage()
		{
			...
			messageWriter.WriteMethodCallHeader(_destination, _path, "org.fcitx.Fcitx.InputContext", "FocusIn");
	...
	public Task FocusOutAsync()
	{
		...
		{
			MessageWriter messageWriter = _connection.GetMessageWriter();
			messageWriter.WriteMethodCallHeader(_destination, _path, "org.fcitx.Fcitx.InputContext", "FocusOut");
			MessageBuffer result = messageWriter.CreateMessage();
			messageWriter.Dispose();
			return result;
		}
	...
	public Task ResetAsync()
	{
		...
		{
			MessageWriter messageWriter = _connection.GetMessageWriter();
			messageWriter.WriteMethodCallHeader(_destination, _path, "org.fcitx.Fcitx.InputContext", "Reset");
			MessageBuffer result = messageWriter.CreateMessage();
			messageWriter.Dispose();
			return result;
		}
	...
	public Task SetCursorRectAsync(int x, int y, int w, int h)
	{
		...
		{
			MessageWriter messageWriter = _connection.GetMessageWriter();
			messageWriter.WriteMethodCallHeader(_destination, _path, "org.fcitx.Fcitx.InputContext", "SetCursorRect", "iiii");
	...
	public Task SetCapacityAsync(uint caps)
	{
		...
		{
			MessageWriter messageWriter = _connection.GetMessageWriter();
			messageWriter.WriteMethodCallHeader(_destination, _path, "org.fcitx.Fcitx.InputContext", "SetCapacity", "u");
	...
	public Task SetSurroundingTextAsync(string text, uint cursor, uint anchor)
	{
		...
		{
			MessageWriter messageWriter = _connection.GetMessageWriter();
			messageWriter.WriteMethodCallHeader(_destination, _path, "org.fcitx.Fcitx.InputContext", "SetSurroundingText", "suu");
	...
	public Task SetSurroundingTextPositionAsync(uint cursor, uint anchor)
	{
		...
		{
			MessageWriter messageWriter = _connection.GetMessageWriter();
			messageWriter.WriteMethodCallHeader(_destination, _path, "org.fcitx.Fcitx.InputContext", "SetSurroundingTextPosition", "uu");
			messageWriter.WriteUInt32(cursor);
			messageWriter.WriteUInt32(anchor);
			MessageBuffer result = messageWriter.CreateMessage();
			messageWriter.Dispose();
	...
	public Task DestroyICAsync()
	{
		...
		{
			MessageWriter messageWriter = _connection.GetMessageWriter();
			messageWriter.WriteMethodCallHeader(_destination, _path, "org.fcitx.Fcitx.InputContext", "DestroyIC");
			MessageBuffer result = messageWriter.CreateMessage();
			messageWriter.Dispose();
			return result;
		}
	...
	public Task<int> ProcessKeyEventAsync(uint keyval, uint keycode, uint state, int type, uint time)
	{
		...
		{
			MessageWriter messageWriter = _connection.GetMessageWriter();
			messageWriter.WriteMethodCallHeader(_destination, _path, "org.fcitx.Fcitx.InputContext", "ProcessKeyEvent", "uuuiu");
	...
	public ValueTask<IDisposable> WatchForwardKeyAsync(Action<Exception?, (uint keyval, uint state, int type)> handler, bool emitOnCapturedContext = true)
	{
		MatchRule rule = new MatchRule
		{
			Type = MessageType.Signal,
		...
		return SignalHelper.WatchSignalAsync(_connection, rule, ReaderExtensions.ReadMessage_uui, handler, emitOnCapturedContext);
	}
