...
public class SKData : SKObject, ISKNonVirtualReferenceCounted, ISKReferenceCounted
{
	...
	public unsafe static SKData Create(string filename)
	{
		...
		fixed (byte* encodedText = StringUtilities.GetEncodedText(filename, SKTextEncoding.Utf8, addNull: true))
		{
			return GetObject(SkiaApi.sk_data_new_from_file(encodedText));
		}
	}
