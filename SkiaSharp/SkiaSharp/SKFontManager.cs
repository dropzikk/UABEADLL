...
public class SKFontManager : SKObject, ISKReferenceCounted
{
	...
	public unsafe SKTypeface MatchFamily(string familyName, SKFontStyle style)
	{
		...
		fixed (byte* encodedText = StringUtilities.GetEncodedText(familyName, SKTextEncoding.Utf8, addNull: true))
		{
			SKTypeface @object = SKTypeface.GetObject(SkiaApi.sk_fontmgr_match_family_style(Handle, new IntPtr(encodedText), style.Handle));
	...
	public unsafe SKTypeface CreateTypeface(string path, int index = 0)
	{
		...
		{
			return SKTypeface.GetObject(SkiaApi.sk_fontmgr_create_from_file(Handle, encodedText, index));
		}
	}
	...
	public unsafe SKTypeface MatchCharacter(string familyName, SKFontStyle style, string[] bcp47, int character)
	{
		...
		fixed (byte* encodedText = StringUtilities.GetEncodedText(familyName, SKTextEncoding.Utf8, addNull: true))
		{
			SKTypeface @object = SKTypeface.GetObject(SkiaApi.sk_fontmgr_match_family_style_character(Handle, new IntPtr(encodedText), style.Handle, bcp47, (bcp47 != null) ? bcp47.Length : 0, character));
			@object?.PreventPublicDisposal();
			return @object;
		}
	}
