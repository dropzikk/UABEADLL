...
public readonly struct SKColor : IEquatable<SKColor>
{
	...
	public static bool TryParse(string hexString, out SKColor color)
	{
		...
		case 4:
		{
			...
			if (length == 4)
			{
				if (!byte.TryParse(string.Concat(hexString[length - 4], hexString[length - 4]), NumberStyles.HexNumber, CultureInfo.InvariantCulture, out result2))
				{
					color = Empty;
					return false;
				}
			...
			if (!byte.TryParse(string.Concat(hexString[length - 3], hexString[length - 3]), NumberStyles.HexNumber, CultureInfo.InvariantCulture, out var result3) || !byte.TryParse(string.Concat(hexString[length - 2], hexString[length - 2]), NumberStyles.HexNumber, CultureInfo.InvariantCulture, out var result4) || !byte.TryParse(string.Concat(hexString[length - 1], hexString[length - 1]), NumberStyles.HexNumber, CultureInfo.InvariantCulture, out var result5))
			{
				color = Empty;
				return false;
			}
