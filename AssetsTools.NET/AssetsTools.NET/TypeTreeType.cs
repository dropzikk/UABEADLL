...
public class TypeTreeType
{
	...
	public string StringBuffer
	{
		get
		{
			return Encoding.UTF8.GetString(StringBufferBytes);
		}
		set
		{
			StringBufferBytes = Encoding.UTF8.GetBytes(value);
		}
	}
	...
	public void Read(AssetsFileReader reader, uint version, bool hasTypeTree, bool isRefType)
	{
		...
		if ((version < 17 && TypeId < 0) || (version >= 17 && TypeId == 114) || (isRefType && ScriptTypeIndex != ushort.MaxValue))
		{
		...
		Nodes = new List<TypeTreeNode>(num);
		for (int i = 0; i < num; i++)
		{
		...
		StringBufferBytes = reader.ReadBytes(count);
		if (version < 21)
		{
			return;
		}
		if (!isRefType)
		{
			...
			TypeDependencies = new int[num2];
			for (int j = 0; j < num2; j++)
			{
				TypeDependencies[j] = reader.ReadInt32();
			}
		}
		else
		{
	...
	public void Write(AssetsFileWriter writer, uint version, bool hasTypeTree)
	{
		...
		if (!IsRefType)
		{
			...
			for (int j = 0; j < TypeDependencies.Length; j++)
			{
				writer.Write(TypeDependencies[j]);
			}
		}
		else
		{
