...
public class WasmTypeEntry
{
	...
	public WasmTypeEntry(WasmFile file)
	{
		...
		ParamTypes = (from b in file.ReadByteArrayAtRawAddress(file.Position, (int)ParamCount)
			select (WasmTypeEnum)b).ToArray();
		ReturnCount = file.BaseStream.ReadLEB128Unsigned();
		ReturnTypes = (from b in file.ReadByteArrayAtRawAddress(file.Position, (int)ReturnCount)
			select (WasmTypeEnum)b).ToArray();
	}
}
