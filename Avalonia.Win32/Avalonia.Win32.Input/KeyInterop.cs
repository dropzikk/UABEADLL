...
public static class KeyInterop
{
	...
	private static int GetVirtualKey(int virtualKey, int keyData)
	{
		if (virtualKey == 16)
		{
			virtualKey = (int)UnmanagedMethods.MapVirtualKey((uint)((keyData & 0xFF0000) >> 16), 3u);
			if (virtualKey == 0)
			{
				virtualKey = 160;
			}
		...
		if (virtualKey == 18)
		{
			virtualKey = ((!IsExtended(keyData)) ? 164 : 165);
		}
		if (virtualKey == 17)
		{
			virtualKey = ((!IsExtended(keyData)) ? 162 : 163);
		}
		return virtualKey;
	}
	...
	public static Key KeyFromVirtualKey(int virtualKey, int keyData)
	{
		...
		s_keyFromVirtualKey.TryGetValue(virtualKey, out var value);
		return value;
	}

	public static int VirtualKeyFromKey(Key key)
	{
		s_virtualKeyFromKey.TryGetValue(key, out var value);
		return value;
	}
}
